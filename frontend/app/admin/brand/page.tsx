...
import { Plus, Search, Tag, Edit2, Trash2, RefreshCw, X, CheckCircle2, AlertCircle } from 'lucide-react';
...
export default function BrandPage() {
  ...
  const [formData, setFormData] = useState({ name: '', logo: '', description: '', status: 'ACTIVE' });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setFormError(err.response?.data?.message || 'Gagal menyimpan data brand');
    } finally {
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-6 rounded-xl shadow-sm border border-slate-100">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">Kelola daftar merek perangkat elektronik yang didukung.</p>
        </div>
        <button
      ...
      <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-100 flex flex-col sm:flex-row gap-4 items-center justify-between">
        <div className="relative w-full sm:w-80">
          ...
          <input
            type="text"
            placeholder="Cari nama atau deskripsi brand..."
            value={search}
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden">
        {loading ? (
          ...
          <div className="text-center py-16 px-4">
            ...
            <p className="text-slate-600 font-medium">Tidak ada data brand ditemukan</p>
            <p className="text-sm text-slate-400 mt-1">Coba sesuaikan kata kunci pencarian atau tambah brand baru.</p>
          </div>
        ) : (
                  ...
                  <tr key={brand.id} className="hover:bg-slate-50/80 transition-colors">
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-lg bg-slate-100 border border-slate-200 flex items-center justify-center font-bold text-slate-600 overflow-hidden">
                          {brand.logo ? (
                    ...
                    <td className="px-6 py-4 text-slate-500 text-xs">
                      {new Date(brand.createdAt).toLocaleDateString('id-ID', { day: 'numeric', month: 'short', year: 'numeric' })}
                    </td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
          <div className="bg-white rounded-2xl max-w-md w-full p-6 shadow-xl border border-slate-100 space-y-4 animate-in fade-in zoom-in duration-150">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                <label className="block text-xs font-semibold text-slate-700 uppercase tracking-wider mb-1">Nama Brand *</label>
                <input
                  type="text"
                  required
              ...
              <div>
                ...
                <textarea
                  rows={3}
                  placeholder="Keterangan singkat tentang merek ini..."
      ...
      {deleteId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
          <div className="bg-white rounded-2xl max-w-sm w-full p-6 shadow-xl border border-slate-100 space-y-4 text-center">
            ...
            <div>
              <h3 className="text-lg font-bold text-slate-900">Hapus Brand?</h3>
