...
import { Search, Map, Clock, ArrowRight, Eye, CheckCircle2, ShieldAlert, Monitor, Wrench, RefreshCw } from 'lucide-react';
...
export default function AdminTrackingPage() {
  ...
  return (
    <div className="space-y-6 max-w-5xl mx-auto">
      ...
      {/* Info Box */}
      <div className="p-4 rounded-xl bg-blue-50 border border-blue-100 text-blue-800 text-xs flex items-center gap-3">
        <Clock className="h-5 w-5 text-blue-600 flex-shrink-0" />
        <div>
          <span className="font-bold">Prinsip Single Source of Truth:</span> Seluruh pembaruan status dan penugasan teknisi dilakukan dari menu <a href="/admin/booking" className="underline font-bold text-blue-900">Booking</a>. Halaman tracking ini bersifat viewer (read-only) untuk menginspeksi hasil visualisasi ke pelanggan.
        </div>
      </div>
        ...
        <form onSubmit={handleSearch} className="flex flex-col sm:flex-row gap-3">
          <div className="relative flex-grow">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2.5 text-sm bg-slate-50 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
            />
          </div>
          <button
            type="submit"
            disabled={loading}
            className="px-5 py-2.5 bg-blue-600 hover:bg-blue-700 font-semibold text-white text-sm rounded-lg shadow-sm transition-all disabled:opacity-50 flex items-center justify-center gap-2 cursor-pointer"
          >
      ...
      {trackingData && (
        <div className="bg-white rounded-xl shadow-sm border border-slate-100 p-6 space-y-6 animate-in fade-in duration-150">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center border-b border-slate-100 pb-4 gap-2">
            <div>
              <span className="text-xs text-slate-400 font-semibold uppercase">Kode Booking</span>
              <h2 className="text-xl font-bold font-mono text-slate-900">{trackingData.bookingNumber}</h2>
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-xs">
            <div className="p-3 bg-slate-50 rounded-lg border border-slate-200 space-y-1">
              <span className="text-slate-400 font-semibold uppercase block">Pelanggan</span>
            ...
            <div className="p-3 bg-slate-50 rounded-lg border border-slate-200 space-y-1">
              <span className="text-slate-400 font-semibold uppercase block">Perangkat</span>
            ...
            <div className="p-3 bg-slate-50 rounded-lg border border-slate-200 space-y-1">
              <span className="text-slate-400 font-semibold uppercase block">Teknisi Bertugas</span>
          ...
          <div className="space-y-4 pt-2">
            <h3 className="text-sm font-bold text-slate-900 flex items-center gap-2">
              <Clock className="h-4 w-4 text-blue-600" /> Log Histori Tracking
            </h3>
